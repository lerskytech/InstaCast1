      ...
      <div className="host-content">
        ...
        <div className="guests-section">
          <div className="guests-header">
            ...
            <button className="share-button" onClick={toggleQRModal}>Share Session</button>
          </div>
          ...
          {connectedGuests.length === 0 ? (
            ...
            <ul className="guests-list">
              {connectedGuests.map(guest => (
                <li key={guest.id} className={`guest-item ${!guest.connected ? 'disconnected' : ''}`}>
                  <div className="guest-info">
                    <span className="guest-id">{guest.id.substring(0, 6)}</span>
                    <span className={`guest-status ${guest.connected ? 'connected' : 'disconnected'}`}>
